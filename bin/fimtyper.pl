...
use constant PROGRAM_NAME            => 'FimTyper-1.1.pl';
use constant PROGRAM_NAME_LONG       => 'Findes FimH types for a sequence or genome';
use constant VERSION                 => '1.1';

my ($Help, $threshold, $min_length, $InFile, $dir, $BLAST, $BLASTN, $MAKEDB, $ABRES_DB);
my $IFormat = "fasta";
my $OFormat = "ST";
...
if (defined $Help || not defined $InFile || not defined $threshold || not defined $min_length) {
   print_help();
   exit;
}

if (not -f $InFile) {
  print "The input file provided $InFile does not exists!\n";
  exit;
}
...
my $best = ""; # will contain best fimtype
my $best_ID = 0; # will contain ID for best fimtype
my $best_cov = 0; # will contain coverage for best fimtype

### Variables for text-printing ###
my $txtresults= "FimTyper result file\n\n"; #Added to txt print
my $tabr = ''; #added to print tab-separated txt file
my  $contigtable ="\nContig Table\n"; #added to txt print
$contigtable .= "-----------\n";
my $alignment = "\n\nAlignments as text\n_________________________________________________________________________________\n";
my @headarray = ("Fimtype      ", "IDENTITY     ", "QUERY/HSP     ", "CONTIG  ", "POSITION         ", "ACC NO.         ");
...
=cut print "\n";
  ...
  if (!%GENE_RESULTS_HASH) {
    ...
    foreach my $key (sort keys %GENE_RESULTS_HASH) {
      ...
      if (@$array[1] > $best_ID) {
        $best = @$array[9];
        $best_ID = @$array[1];
        $best_cov = @$array[2] / @$array[3]; 
      }
      
    }
    if ($best_ID != 100 or $best_cov != 1) {
      $tabr .= "FimH type: Unknown or presumptive new variant *\n";
      $tabr .= "Closest match: $best-like\n";
    }
    else {
      $tabr .= "FimH type: $best\n\n";
    }
    
    $tabr .= $argfProfiles{"$AB_indput"}. "\nFimtype\tIdentity\tQuery/HSP\tContig\tPosition in contig\tAccession no.\n"; #added to print tab-separated txt file
  }
  ...
  foreach my $key (sort keys %GENE_RESULTS_HASH) {
    ...
    $tabr .= @$array[9]."\t".@$array[1]."\t".@$array[2]."/".@$array[3]."\t".@$array[4]."\t".@$array[5]."\t".@$array[8]."\n";
  }
  
  if ($best_ID != 100 or $best_cov != 1) {
    $tabr .= "Please contact curator, if new fimH-type is suspected";
  }
...
foreach my $key (sort keys %FINAL_RESULTS) {
  ...
  $txtresults .= $header."\n" ;
  $txtresults .="*********************************************************************************\n";
  $txtresults .="FimH TYPE      IDENTITY     QUERY/HSP     CONTIG  POSITION         ACC NO.         \n";
  $txtresults .="=================================================================================\n";
...
foreach my $key (sort keys %GENE_RESULTS_HASH2) {
  ...
  for (my $i=0; $i < scalar(@$hitArray); $i++){
	 ...
	 $alignment .= "Fimtype seq:         ".$tmpQuerySingleLine."\n";
...
open (ALLELE, '>'.$dir.'/Fimtype_seq.fsa') || die("Error! Could not write to Fimtype_seq.fsa");
...
sub get_blast_run {
   ...
   system("$BLASTN -db $tmp_dir/$file -query $tmp_dir/$query_file -dust 'no' -out $file2");
...
DESCRIPTION
  ...
  FimTyper alignment. Although the user can change the arguments just as if he
